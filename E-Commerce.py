...
----------------------------------------------------------------------------------
Requires: mysql-connector-python, pandas, numpy
Install: pip install mysql-connector-python, pandas, numpy
Modify DB_USER and DB_PASS below to match your MySQL credentials.
"""
...
def sales_analytics():
    cursor.execute("""
        SELECT p.name, oi.unit_price, oi.quantity, (oi.unit_price * oi.quantity) as line_total
        FROM order_items oi
    ...
    arr = np.array([d[3] for d in data], dtype=float)  # line_totals
    product_names = [d[0] for d in data]

    revenue = np.sum(arr)
    avg_order_value = np.mean(arr)
    total_items = np.sum([d[2] for d in data])

    # Stats
    max_sale = np.max(arr)
    min_sale = np.min(arr)
    median_sale = np.median(arr)
    std_dev = np.std(arr)
    ...
    print("Average Sale Value:", round(avg_order_value, 2))
    print("Total Items Sold:", int(total_items))
    print(f"Max Single Sale: ₹{max_sale} | Product: {product_names[max_index]}")
    print(f"Min Single Sale: ₹{min_sale} | Product: {product_names[min_index]}")
    print(f"Median Sale Value: ₹{median_sale} (Represents middle sale)")
    print(f"Standard Deviation: {round(std_dev,2)} (Spread of sales)")
    print(f"Percentiles: 25%={round(percentiles[0],2)}, 50%={round(percentiles[1],2)}, 75%={round(percentiles[2],2)}")


def low_stock_alert(threshold=5):
    cursor.execute("SELECT product_id, name, stock FROM products WHERE stock <= %s", (threshold,))
    rows = cursor.fetchall()
    print(f"\nLow Stock Products (≤ {threshold} units):")
    for r in rows:
        print(f"ID:{r[0]} | {r[1]} | Stock:{r[2]}")
    if not rows:
...
def top_selling_products():
    ...
    for idx, r in enumerate(rows, 1):
        print(f"{idx}. {r[0]} — {int(r[1])} sold")
    if not rows:
        print("No sales data available yet.")

def customer_purchase_history():
    list_customers()
    cust_id = int(input("Enter Customer ID to view history: ").strip())
    cursor.execute("""
    ...
    print(f"\nPurchase History for Customer ID {cust_id}:")
    if not orders:
        print("No orders found for this customer.")
        return
    
    for order in orders:
        print(f"OrderID:{order[0]} | Date:{order[1]} | Total:₹{to_decimal(order[2])} | Status:{order[3]}")
        cursor.execute("""
            SELECT p.name, oi.quantity, oi.line_total
            FROM order_items oi 
            JOIN products p ON oi.product_id = p.product_id
            WHERE oi.order_id=%s
        """, (order[0],))
        items = cursor.fetchall()
        for it in items:
            print(f"   - {it[0]} | Qty:{it[1]} | Line:₹{to_decimal(it[2])}")

def category_revenue_report():
    cursor.execute("""
        SELECT c.name AS category, oi.quantity, oi.unit_price, (oi.quantity*oi.unit_price) AS line_total
        FROM order_items oi
        JOIN products p ON oi.product_id = p.product_id
    ...
    df = pd.DataFrame(data, columns=['Category', 'Quantity', 'UnitPrice', 'LineTotal'])
    
    # Replace None category with 'Uncategorized'
    df['Category'] = df['Category'].fillna('Uncategorized')
    
    # Aggregate revenue per category
    category_revenue = df.groupby('Category')['LineTotal'].sum().sort_values(ascending=False)
    
    print("\nRevenue by Category:")
    print(category_revenue)
    
    # Optional: export to Excel
    category_revenue.to_excel("category_revenue.xlsx")
    print("Excel report generated: category_revenue.xlsx")
...
def monthly_sales():
    ...
    df = pd.DataFrame(data, columns=['OrderID', 'OrderDate', 'TotalAmount'])
    ...
    # Group by MonthYear
    monthly_revenue = df.groupby('MonthYear')['TotalAmount'].sum()

    print("\n📊 Monthly Sales Trend:")
    print(monthly_revenue)

    # Export to Excel
    monthly_revenue.to_excel("monthly_sales.xlsx")
    print("Excel report generated: monthly_sales.xlsx")
...
def main_menu():
    while True:
        ...
        time.sleep(0.2)
        print("15. Sales Analytics")
        time.sleep(0.2)
        print("16. Low Stock Alerts")
        time.sleep(0.2)
        print("17. Top 5 Best-Selling Products")
        time.sleep(0.2)
        print("18. Customer Purchase History")
        time.sleep(0.2)
        print("19. Category-wise Revenue Report")
        time.sleep(0.2)
        print("20. Montly Sales Trend")
        time.sleep(0.2)
        print("0. Exit")
        time.sleep(0.2)
        print("\n")
